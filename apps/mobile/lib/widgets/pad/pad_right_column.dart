    ...
    if (mode == WorkspaceMode.review) {
      final changes = [
        for (final file in gitStatus?.changedFiles ?? const <RemoteGitFileStatus>[])
          _ReviewChangeEntry(_reviewStatusCode(file), file.path),
      ];
...
class _ReviewFileTreeState extends State<_ReviewFileTree> {
  ...
  Widget build(BuildContext context) {
    final accent = Theme.of(context).colorScheme.secondary;
    final prefs = AppPreferences.of(context);
    final snapshot = PadDirectorySnapshot<_ReviewChangeEntry>.from(
      _currentPath,
      widget.changes,
    );
    final parentPath = _currentPath.isEmpty
        ? null
        : padParentPath(_currentPath);
    final rows = <Widget>[
      if (parentPath != null)
        PadFileListItem(
          icon: Icons.arrow_upward_rounded,
      ...
      for (final file in snapshot.files)
        PadFileListItem(
          ...
          trailing: PadStatusTag(
            label: file.status,
            color: padStatusColor(file.status, accent),
          ),
...
class _ReviewChangeEntry implements PadTreeEntry {
  const _ReviewChangeEntry(this.status, this.path);
...
Future<String?> _promptMessage(BuildContext context, String title) async {
  ...
  // Dispose after the exit animation so the field never uses a disposed
  // controller while it animates out.
