...
<p align="center">
  ...
  用 <b>Rust + GPUI</b> 原生打造，统一 Codex、Claude Code 等 8+ AI 编程 CLI——实时状态、Token 统计、本地记忆、安全 SSH，外加桌面 ⇄ 手机 ⇄ 无头主机 的互联，让你随时随地接管长 agent 任务。
</p>
...
| 代码在另一台机器上 | 连上一个无头主机——服务器、闲置的 Mac 或 Linux——像操作本地一样驱动它的终端、Git 和 AI。 |
...
> **Beta。** 连接无头主机会先在本次版本里以 beta 形式上线——连接、配对、主机侧数据链路都还在持续测试中，可能会有粗糙的地方，欢迎反馈。

桌面端、手机、无头主机互为 **peer**，经端到端加密的 Iroh 链路连通，让你随时随地接着跑长 agent 任务。

- **手机接力。** 扫码几秒配对，自动选最快路径、连不上走中继兜底。项目、终端、文件、AI 都跑在主机上，手机只负责操控；切过去也能完整看到终端历史。
- **无头主机。** 把 `codux` agent 跑在服务器、闲置的 Mac 或 Linux 上，像本地一样驱动它的终端、Git、AI——所有功能都跑在该主机自己的数据上。详见 [`apps/agent/README.md`](apps/agent/README.md)。
...
Codux 用 **Rust + GPUI** 打造——和 [Zed](https://zed.dev) 同源的原生技术，所以终端渲染、项目切换、长时间高强度的 agent 输出都又快又稳，不臃肿、不吃内存。桌面端、移动端、无头主机共用**同一套 Rust 终端内核**，每个设备渲染出的会话完全一致——覆盖 macOS、Windows 与 Linux。
