...
- **AI 工具记不住项目上下文** — 有用的决策、约定和问题经验很容易埋在终端历史里
...
Codux 可以从已完成的 AI 编程会话中提取长期记忆，把用户偏好、项目约定、技术决策、事实信息和问题经验保存到本地 SQLite 数据库中。之后启动 Claude、Codex、Gemini、OpenCode 等受支持工具时，Codux 会在运行时支持目录中生成简洁上下文，并与全局提示词一起注入给工具使用，不会把记忆文件写进你的项目根目录。

AI 记忆亮点：

- 使用本地 `memory.sqlite3` 保存用户记忆和项目记忆
- 自动从已完成的 Claude、Codex、Gemini、OpenCode 会话中提取记忆
- 支持“自动”提取供应商，优先使用当前终端工具对应供应商，再按优先级回退
- 每个供应商都有测试按钮，方便确认模型、CLI 或 API 凭据可用
- 支持全局提示词，启动受支持工具时自动合并进上下文
- 通过应用私有的 `CLAUDE.md`、`AGENTS.md`、`GEMINI.md` 生成启动上下文
- 顶部栏会显示记忆提取的排队、运行和失败状态
